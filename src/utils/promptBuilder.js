function buildPrompt(commits, metrics, { role, projectName, startDate, endDate }) {
  ...
  const metricLines = [
    ...
    `Avg commits per day: ${(
      Object.values(metrics.commitsPerDay).reduce((a, b) => a + b, 0) /
      Math.max(Object.keys(metrics.commitsPerDay).length, 1)
    ).toFixed(2)}`,
